...
using TriggerVariant =
  std::variant<std::monostate, tests::EmptyTrigger, tests::BatteryHealthTrigger>;

class TriggeredRecorderNodeTestHelper : public TriggeredRecorderNode<TriggerVariant>
{
public:
  TriggeredRecorderNodeTestHelper(
    std::string && node_name = "triggered_recorder_node_test_helper",
  ...
  std::unordered_map<std::string, TriggerVariant> get_triggers() const { return triggers_; }
  size_t get_number_of_subscriptions() const
  {
    return triggered_subscriptions_.size() + untriggered_subscriptions_.size();
  }

  YAML::Node get_topics_config() const { return topics_config_; }

  std::filesystem::path get_bag_path() { return get_writer_impl().get_base_folder(); }

  std::filesystem::path get_root_dir() { return get_writer_impl().get_config().bag_root_dir; }

  bool is_writer_initialized()
  {
    auto & triggered_writer = get_writer_impl();
    auto storage_opts = triggered_writer.get_storage_options();
    return triggered_writer.is_open() && !triggered_writer.get_bag_name().empty() &&
           !storage_opts.uri.empty();
  }
  ...
  bool is_trigger_buffer_timer_initialized() const { return trigger_buffer_timer_ != nullptr; }

  std::pair<int64_t, int64_t> get_crop_points() const { return crop_points_; }

  void test_init(
    size_t expected_topics_recorded, size_t expected_subscriptions,
    std::vector<std::string> expected_trigger_types)
  {
    ...
    {
      if (
        std::find(expected_trigger_types.begin(), expected_trigger_types.end(), trigger.first) ==
        expected_trigger_types.end())
      {
  ...
  void publish_battery_msg(bool is_activated)
  {
    ...
    auto battery_status = is_activated ? BatteryState::POWER_SUPPLY_HEALTH_DEAD
...
protected:
  ...
  void SetUp(const std::string & config_dir)
  {
    ...
    test_helper_ = std::make_shared<TriggeredRecorderNodeTestHelper>(
      "triggered_recorder_node_test_helper", rclcpp::NodeOptions(), config_dir);
  }
...
TEST_F(TriggeredRecorderNodeTestFixture, test_valid_initialization)
{
  ...
  const size_t expected_subscriptions =
    3;  //Topics that are neither triggered nor recorded are not added to subscriptions.
...
TEST_F(TriggeredRecorderNodeTestFixture, test_config_with_invalid_trigger_names)
{
  std::filesystem::path prefix_path = ament_index_cpp::get_package_prefix("ros2bag_triggered");
  EXPECT_THROW(
    SetUp(prefix_path / "test/configs/config_with_invalid_trigger_names"), std::out_of_range);
}
...
TEST_F(TriggeredRecorderNodeTestFixture, test_config_with_missing_triggers)
{
  ...
  const size_t expected_subscriptions =
    2;  //Topics that are neither triggered nor recorded are not added to subscriptions.
...
TEST_F(TriggeredRecorderNodeTestFixture, test_topic_callback)
{
  ...
  EXPECT_EQ(
    crop_points.first,
    std::visit(get_trigger_pulses, triggers.at("BatteryHealthTrigger")).at(0).start_time_);
  EXPECT_EQ(
    crop_points.second,
    std::visit(get_trigger_pulses, triggers.at("EmptyTrigger")).at(0).end_time_);
}
