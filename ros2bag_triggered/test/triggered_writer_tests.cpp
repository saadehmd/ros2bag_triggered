...
public:
  ...
  rosbag2_storage::BagMetadata get_metadata() { return metadata_; }

  ros2bag_triggered::TriggeredWriter::Config get_config() const { return config_; }
  ...
  bool is_initialized() const { return !bag_name_.empty(); }

  void set_metadata(const rosbag2_storage::BagMetadata & metadata) { metadata_ = metadata; }

  void set_config(const ros2bag_triggered::TriggeredWriter::Config & config) { config_ = config; }
  ...
  void set_triggered(bool is_triggered) { is_triggered_ = is_triggered; }
};
...
protected:
  ...
  : writer_{std::make_unique<ros2bag_triggered::TriggeredWriter>(
      rclcpp::get_logger("TriggeredWriterTests"))}
  {
  }
  ...
  ~TriggeredWriterTestFixture()
  {
    ...
    if (test_helper.is_initialized())
    {
      ...
      const auto triggered_bag_path =
        get_test_helper().get_config().bag_root_dir + "/triggered_bags";
  ...
  std::string open_valid_bag()
  {
    ...
    std::string bag_name =
      "test_bag_" + std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
  ...
  void test_close_with_triggers(bool do_crop)
  {
    ...
    writer_.write<std_msgs::msg::Bool>(
      std_msgs::msg::Bool(), "test_topic", rclcpp::Time(storage_options.start_time_ns));
    writer_.write<std_msgs::msg::Bool>(
      std_msgs::msg::Bool(), "test_topic",
      rclcpp::Time(storage_options.start_time_ns + expected_duration_ns / 2));
    writer_.write<std_msgs::msg::Bool>(
      std_msgs::msg::Bool(), "test_topic",
      rclcpp::Time(storage_options.start_time_ns + expected_duration_ns));
    ...
    EXPECT_NEAR(
      bag_meta.starting_time.time_since_epoch().count(),
      do_crop ? storage_options.start_time_ns + expected_duration_ns / 2
              : storage_options.start_time_ns,
      1000);
  }
...
TEST_F(TriggeredWriterTestFixture, test_invalid_config_filenames)
{
  ...
  EXPECT_THROW(
    test_helper.initialize(
      prefix_path / "test/configs/invalid_filenames_config/writer_config.yaml"),
    YAML::BadFile);
}
...
TEST_F(TriggeredWriterTestFixture, test_incomplete_config)
{
  ...
  auto & test_helper = get_test_helper();
  EXPECT_NO_THROW(
    test_helper.initialize(prefix_path / "test/configs/incomplete_config/writer_config.yaml"));
}
...
TEST_F(TriggeredWriterTestFixture, test_config_with_invalid_field_types)
{
  ...
  EXPECT_THROW(
    test_helper.initialize(
      prefix_path / "test/configs/config_with_wrong_field_types/writer_config.yaml"),
    YAML::BadConversion);
}
...
TEST_F(TriggeredWriterTestFixture, test_open)
{
  ...
  EXPECT_TRUE(
    test_helper.get_base_folder() == test_helper.get_config().bag_root_dir + "/" + bag_name);
...
TEST_F(TriggeredWriterTestFixture, test_close_no_triggers)
{
  ...
  EXPECT_FALSE(
    std::filesystem::exists(test_helper.get_config().bag_root_dir + "/triggered_bags/" + bag_name));
}
...
TEST_F(TriggeredWriterTestFixture, test_cropping)
{
  ...
  EXPECT_EQ(
    rewrite_options.value().start_time_ns, static_cast<double>(trigger_start_time) - crop_gap);
