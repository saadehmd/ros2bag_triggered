...
void TriggeredWriter::set_crop_points(int64_t start_time, int64_t end_time)
{
  ...
  uint64_t bag_end_time =
    bag_start_time + duration_cast<std::chrono::nanoseconds>(metadata_.duration).count();
...
void TriggeredWriter::initialize(const std::filesystem::path & writer_config)
{
  ...
  if (writer_cfg["write_trigger_stats"])
...
void TriggeredWriter::close()
{
  ...
  if (do_crop)
  {
    ...
    RCLCPP_WARN(
      logger_, "Cropping enabled. Rewriting triggered bag to: %s ...",
      rewrite_options_->uri.c_str());

    rosbag2_transport::bag_rewrite(
      {storage_options_}, {std::make_pair(*rewrite_options_, record_options)});
    std::filesystem::remove_all(base_folder);
  } else
  {
    RCLCPP_WARN(
      logger_, "Cropping is disabled or not needed. Moving the triggered bag to: %s ...",
...
void TriggeredWriter::write_trigger_stats(
...
void TriggeredWriter::plot_triggers(const TriggerPulseMap & trigger_pulses)
{
  ...
  if (config_.write_trigger_stats && is_triggered_)
  {
    TimePointNs bag_start_time{
      rewrite_options_ ? NanoSec(rewrite_options_->start_time_ns)
                       : metadata_.starting_time.time_since_epoch()};
    TimePointNs bag_end_time{
      rewrite_options_ ? NanoSec(rewrite_options_->end_time_ns)
                       : metadata_.starting_time.time_since_epoch() + metadata_.duration};

    using path = std::filesystem::path;
    std::filesystem::path triggered_bag_path =
      path(config_.bag_root_dir) / "triggered_bags" / bag_name_;
