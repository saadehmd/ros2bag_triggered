  ...
  if constexpr (!std::is_same_v<std::decay_t<decltype(trigger)>, std::monostate>)
  {
    ...
    } else
    {
      throw std::runtime_error(
        "Mismatch between msg-type on trigger: " + trigger.get_msg_type() +
...
auto configure_trigger = [](auto & trigger, const YAML::Node & config) {
  if constexpr (!std::is_same_v<std::decay_t<decltype(trigger)>, std::monostate>)
  {
    ...
    {
      throw std::runtime_error(
        "Trigger type: " + trigger.get_name() +
        " is already registered to a topic. Registering single trigger-type to multiple topics is "
        "not yet supported.");
    }
