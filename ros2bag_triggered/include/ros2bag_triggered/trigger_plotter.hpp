...
{
  return (a.trigger_pulse_.end_time_ - a.trigger_pulse_.start_time_) >
         (b.trigger_pulse_.end_time_ - b.trigger_pulse_.start_time_);
}

inline void sort_trigger_pulses(
  const TriggerPulseMap & trigger_pulse_map, TriggerPulsePlots & trigger_pulse_plots)
{
  trigger_pulse_plots.reserve(trigger_pulse_map.size());
  // 16-bit RGB color represented in hex by 0xFFFFFF, for using brighter colors
  constexpr int max_colors_16bit{16777215 / 2};
  // 24-bit RGB color represented in hex by 0xFFFFFF, for using full color range
  constexpr int max_colors_24bit{16777215};

  for (const auto & [trigger_name, trigger_pulses] : trigger_pulse_map)
  {
    std::string color =
      "#" + to_hex(trigger_pulse_map.hash_function()(trigger_name) % max_colors_16bit);
  ...
  double time_axs_resolution, size_t time_axis_size)
{
  ...
  auto trigger_end_time = std::chrono::duration_cast<std::chrono::seconds>(
    TimePoint(Ns(trigger_pulse.end_time_)) - bag_start_time);
...
inline void plot_triggers(
  const TriggerPulseMap & trigger_pulse_map, const TimePoint<Ns> & bag_start_time,
  const TimePoint<Ns> & bag_end_time, const std::filesystem::path & trigger_plot_file)
{
  double bag_duration =
    std::chrono::duration_cast<std::chrono::seconds>(bag_end_time - bag_start_time).count();
  ...
  try
  {
    ...
    for (const auto & trigger_pulse_plot : trigger_pulse_plots)
    {
      ...
      plot_fill_calls.push_back([&trigger_pulse_plot, x_fill, y_fill]() {
        matplotlibcpp::fill_between(
          x_fill, std::vector<double>(x_fill.size(), 0.95), y_fill,
          {{"color", trigger_pulse_plot.trigger_color_}});
        matplotlibcpp::fill_between(
          x_fill, std::vector<double>(x_fill.size(), 0.0), y_fill,
          {{"color", trigger_pulse_plot.trigger_color_}});
      });
    }
    ...
    matplotlibcpp::suptitle(
      "Bag Start Time: " + start_datetime_str + "\nBag End Time: " + end_datetime_str,
