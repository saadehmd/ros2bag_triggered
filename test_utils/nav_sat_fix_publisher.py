...
class NavSatFixPublisher(Node):
    ...
    def __init__(self, fix_invalid, inside_zone):
...
def main(args=None):
    ...
    parser.add_argument('--fix_invalid', action='store_true', help='Generate invalid GPS fix')
    parser.add_argument('--inside_zone', action='store_true', help='Generate GPS fix within the specified range')
    args = parser.parse_args()

    rclpy.init()
    navsatfix_publisher = NavSatFixPublisher(fix_invalid=args.fix_invalid, inside_zone=args.inside_zone)
